...
class GradCAM:
    ...
    def generate(self, input_tensor: torch.Tensor) -> np.ndarray:
        ...
        # Use absolute gradient strength to capture all influential channels.
        ...
        # Normalize to [0, 1] and boost contrast for strong regions
        cam = cam.squeeze().cpu().numpy()
        cam_min, cam_max = cam.min(), cam.max()
        if cam_max - cam_min > 1e-8:
            cam = (cam - cam_min) / (cam_max - cam_min)
            low_cut = np.percentile(cam, 25)
            cam = np.clip((cam - low_cut) / (1.0 - low_cut + 1e-8), 0, 1)
        else:
...
def apply_colormap(heatmap: np.ndarray, image_bgr: np.ndarray, colormap: int = cv2.COLORMAP_JET, alpha: float = 0.45) -> np.ndarray:
    """
