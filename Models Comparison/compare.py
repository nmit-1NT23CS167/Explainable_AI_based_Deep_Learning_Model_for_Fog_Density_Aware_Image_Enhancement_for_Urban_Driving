...
from models.classical_dehaze import dehaze_dcp
...
def _post_process(bgr: np.ndarray, model_name: str) -> np.ndarray:
    """Refine neural outputs with model-specific haze removal strength."""
    if model_name == 'AOD-Net':
        bgr = dehaze_dcp(bgr)
    elif model_name == 'GridDehazeNet':
        bgr = dehaze_dcp(bgr, omega=0.78, minimum_transmission=0.16)
...
def _find_reference(foggy_path: str, clear_dir: str):
    """
    Match foggy image with its corresponding clear image.

    Example:
        foggy/001.png -> clear/001.png
        foggy/002.jpg -> clear/002.jpg
    """

    # Get filename without extension
    number = os.path.splitext(os.path.basename(foggy_path))[0]

    # Check if filename exists
    if not number:
        print(
            f"[WARNING] Cannot find image number: "
...
def run_comparison(args: argparse.Namespace) -> None:
    ...
    for idx, fp in enumerate(paths):
        ...
        for model_name, model in models.items():
            ...
            enhanced = _post_process(_to_bgr(out_t), model_name)
    ...
    for fp in paths[:50]:   # first 50 images
        from pathlib import Path
        img_name = Path(fp).stem
        foggy    = cv2.imread(fp)
        if foggy is None:
