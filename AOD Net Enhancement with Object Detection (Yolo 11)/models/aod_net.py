...
class AODNetEnhancer:
    ...
    def _load(self, path):
        ...
        if not path or not os.path.exists(path):
            print("[AOD-Net] No weights found — using natural-color fallback")
            return
    ...
    def enhance(self, frame_bgr: np.ndarray) -> np.ndarray:
        # Random weights can create strong false colours, so never display them.
        if not self.weights_loaded:
            return self._fallback_enhance(frame_bgr)
    ...
    @staticmethod
    def _fallback_enhance(frame_bgr: np.ndarray) -> np.ndarray:
        """Improve local contrast without changing the camera's colour balance."""
